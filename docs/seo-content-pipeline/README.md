...
**Comenzile au fost actualizate la 28 aug 2026** pentru stack-ul Coolify —
vechile `ssh VPSIonos` duceau la baza înghețată la cutover.
...
- ✅ **BG + EL**: bulk OpenAI declanșat pe server (2026-06-10 22:45 UTC) — generează singur toate paginile lipsă. Verificare:
  `PROD_SITE_ID=<SITE_UUID> deploy/prod.sh api GET /api/admin/seo-pages/regenerate-status`
...
5. `deploy/prod.sh sql-file /tmp/seo_insert.sql`
...
```bash
PROD_SITE_ID=86a6e2b2-6f20-45f2-ba14-e46dbd1c12ea \
  deploy/prod.sh api POST /api/admin/seo-pages/regenerate-all '{}'
```
