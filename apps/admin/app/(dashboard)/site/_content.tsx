...
export default function SiteConfigPage() {
  ...
  return (
    <div>
      ...
      {activeTab === 'general' && <GeneralTab siteId={siteId} form={form} setForm={setForm} />}
...
/**
 * Secțiune facturare SmartBill per-site. Tokenul vine mascat ca __MASKED__ când
 * e setat; lăsat necompletat → se păstrează. „Client implicit" e folosit pentru
 * a emite facturile pe persoana ta fizică (ex. facturile istorice) în loc de
 * cumpărătorul real. Butonul „Testează conexiunea" validează credențialele
 * SALVATE (apasă Salvează întâi).
 */
function SmartbillSection({
  siteId,
  form,
  setForm,
}: {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Field label="Token API" description="Lasă necompletat pentru a păstra tokenul existent.">
            <Input
              ...
              placeholder={sb.token === '__MASKED__' ? '••••••••' : 'token...'}
            />
          </Field>
          <Field label="CIF firmă (companyVatCode)" description="Fără „RO” pentru neplătitor de TVA.">
            <Input
