...
export const SETTINGS_SCHEMA: SettingCategory[] = [
  ...
  },
  {
    ...
    settings: [
      ...
      {
        key: 'POWERMAIL_API_KEY',
        label: 'Cheie PowerMail',
        kind: 'secret',
        encrypted: true,
        hotReload: true,
        ...
        group: 'PowerMail',
        helpWhat:
          'Tot mailul tranzacțional și de marketing (magic link, livrare, chitanțe, recovery, campanii). O singură cheie pentru toate site-urile — identitatea se alege din adresa de expeditor a fiecărui site.',
        helpWhere: 'powermail.wingo.ro → proiect → Chei API',
        helpUrl: 'https://powermail.wingo.ro',
      },
      {
  ...
  },
  {
    id: 'mail-system',
    title: 'Email sistem',
    description: 'Magic link, confirmări, recuperare. Nu e Inbox Hub. PowerMail sau SMTP — se arată doar providerul ales.',
    settings: [
      {
        key: 'MAIL_PROVIDER',
        label: 'Provider',
        kind: 'select',
        options: ['powermail', 'smtp'],
        optionLabels: { powermail: 'PowerMail', smtp: 'SMTP' },
        hotReload: true,
        helpWhat: 'Pe unde ies mailurile de sistem. Cheia PowerMail stă la Chei.',
      },
      {
        key: 'MAIL_FROM',
        label: 'From (email)',
      ...
      },
      {
        ...
        helpWhere: 'Panoul hostului de mail (nu PowerMail).',
      },
      {
        key: 'SMTP_SECURE',
        label: 'SMTP SSL/TLS',
      ...
      {
        key: 'POWERMAIL_API_URL',
        label: 'Adresă API PowerMail',
        kind: 'string',
        hotReload: true,
        group: 'PowerMail',
        placeholder: 'https://api.powermail.wingo.ro',
        helpWhat: 'Override de endpoint. Gol = instanța standard.',
      },
      {
